...
static void pim_autorp_rp_free(struct pim_autorp_rp *rp, bool installed)
{
	...
	if (installed) {
		if (pim_rp_del(rp->autorp->pim, rp->addr, rp->grp,
			...
			if (PIM_DEBUG_AUTORP)
				zlog_err("%s: Failed to delete RP %pI4", __func__, &rp->addr);
		}
...
static bool pim_autorp_join_groups(struct interface *ifp)
{
	...
	if (PIM_DEBUG_AUTORP)
		zlog_debug("%s: Joined AutoRP discovery group %pPA on interface %s", __func__, &grp,
			   ifp->name);
	...
	if (pim_socket_join(pim->autorp->sock, grp, pim_ifp->primary_address, ifp->ifindex,
			    pim_ifp)) {
		zlog_err("Failed to join group %pI4 on interface %s", &grp, ifp->name);
		return errno;
	}

	if (PIM_DEBUG_AUTORP)
		zlog_debug("%s: Joined AutoRP announcement group %pPA on interface %s", __func__,
			   &grp, ifp->name);
...
static bool pim_autorp_leave_groups(struct interface *ifp)
{
	...
	if (PIM_DEBUG_AUTORP)
		zlog_debug("%s: Left AutoRP discovery group %pPA on interface %s", __func__, &grp,
			   ifp->name);

	inet_pton(PIM_AF, PIM_AUTORP_ANNOUNCEMENT_GRP, &grp);
	if (pim_socket_leave(pim->autorp->sock, grp, pim_ifp->primary_address, ifp->ifindex,
			     pim_ifp)) {
		zlog_err("Failed to leave group %pI4 on interface %s", &grp, ifp->name);
		return errno;
	}
...
static bool pim_autorp_setup(int fd)
{
	...
	if (setsockopt(fd, PIM_IPPROTO, IP_PKTINFO, &data, data_len)) {
		if (PIM_DEBUG_AUTORP)
			zlog_err("%s: Could not set IP_PKTINFO on socket fd=%d: errno=%d: %s",
	...
	if (set_nonblocking(fd) < 0) {
		if (PIM_DEBUG_AUTORP)
			zlog_err("%s: Could not set non blocking on socket fd=%d: errno=%d: %s",
				 __func__, fd, errno, safe_strerror(errno));
		return false;
	}
	...
	if (sockopt_reuseaddr(fd)) {
		if (PIM_DEBUG_AUTORP)
			zlog_err("%s: Could not set reuse addr on socket fd=%d: errno=%d: %s",
				 __func__, fd, errno, safe_strerror(errno));
		return false;
	}
	...
	if (setsockopt_ipv4_multicast_loop(fd, 1) < 0) {
		if (PIM_DEBUG_AUTORP)
			zlog_err("%s: Could not enable multicast loopback on socket fd=%d: errno=%d: %s",
				 __func__, fd, errno, safe_strerror(errno));
		return false;
	}
	...
	if (bind(fd, (const struct sockaddr *)&autorp_addr, sizeof(autorp_addr)) < 0) {
		if (PIM_DEBUG_AUTORP)
			zlog_err("%s: Could not bind socket: %pSUp, fd=%d, errno=%d, %s", __func__,
				     ...
				     char *buf, size_t buf_size)
{
	...
	for (i = 0; i < rpcnt; ++i) {
		...
		if (rp->pimver == AUTORP_PIM_V1 || rp->pimver == AUTORP_PIM_VUNKNOWN) {
			if (PIM_DEBUG_AUTORP)
				zlog_warn("%s: Ignoring unsupported PIM version (%u) in AutoRP Announcement for RP %pI4",
		...
		if (rp->grpcnt == 0) {
			/* No groups?? */
			if (PIM_DEBUG_AUTORP)
				zlog_warn("%s: Announcement message has no groups for RP %pI4",
		...
		for (j = 0; j < rp->grpcnt; ++j) {
			...
			if (tgrp != NULL) {
				 ...
				 */
				if (PIM_DEBUG_AUTORP)
					zlog_warn("%s: %pFX was duplicated in AutoRP announcement",
						  __func__, &lgrp->grp);
			      ...
			      char *listname, uint16_t holdtime)
{
	...
	if (ret != PIM_SUCCESS && ret != PIM_RP_NO_PATH) {
		zlog_err("%s: Failed to add active RP addr=%pI4, grp=%pFX, grplist=%s", __func__,
				    ...
				    size_t *sz)
{
	...
	frr_each (pim_autorp_rp, &autorp->advertised_rp_list, rp) {
		...
		if (buf_sz < *sz + rp_sz) {
			if (PIM_DEBUG_AUTORP)
				zlog_warn("%s: Failed to pack AutoRP discovery packet, buffer overrun, (%u < %u)",
		...
		if (setsockopt(autorp->sock, IPPROTO_IP, IP_MULTICAST_TTL,
			...
			if (setsockopt(autorp->sock, IPPROTO_IP, IP_MULTICAST_IF,
				...
				if (sendto(autorp->sock, buf, disc_sz, 0,
					...
					zlog_err("%s: Failed to send AutoRP discovery message, errno=%d, %s",
						 __func__, errno, safe_strerror(errno));
			} else if (PIM_DEBUG_AUTORP)
				zlog_err("%s: Failed to set Multicast Interface for sending AutoRP discovery message, errno=%d, %s",
					 __func__, errno, safe_strerror(errno));
		} else if (PIM_DEBUG_AUTORP)
			zlog_err("%s: Failed to set Multicast TTL for sending AutoRP discovery message, errno=%d, %s",
				 __func__, errno, safe_strerror(errno));
	}
				  ...
				  char *buf, size_t buf_size, pim_addr src)
{
	...
	for (i = 0; i < rpcnt; ++i) {
		...
		if ((buf_size - offset) < (AUTORP_GRPLEN * rp->grpcnt)) {
			if (PIM_DEBUG_AUTORP)
				zlog_warn("%s: Buffer underrun parsing groups for RP %pI4 (%u < %u)",
					  __func__, (in_addr_t *)&(rp->addr),
...
static bool autorp_recv_msg(struct pim_autorp *autorp, char *buf, size_t buf_size, pim_addr src)
{
	...
	if (buf_size < AUTORP_HDRLEN) {
		if (PIM_DEBUG_AUTORP)
			zlog_warn("%s: Invalid AutoRP Header size (%u < %u)", __func__,
				  (uint32_t)buf_size, AUTORP_HDRLEN);
		return false;
	}
...
static bool pim_autorp_socket_enable(struct pim_autorp *autorp)
{
	...
	frr_with_privs (&pimd_privs) {
		...
		if (!pim_autorp_setup(fd)) {
			if (PIM_DEBUG_AUTORP)
				zlog_warn("Could not setup autorp socket fd=%d: errno=%d: %s", fd,
...
static bool pim_autorp_socket_disable(struct pim_autorp *autorp)
{
	if (close(autorp->sock)) {
		if (PIM_DEBUG_AUTORP)
			zlog_warn("Failure closing autorp socket: fd=%d errno=%d: %s", autorp->sock,
				  errno, safe_strerror(errno));
		return false;
	}
